...
void spectrumAnalyzer(){
	...
	while(1){
		...
		for(i = 0; i <= zeroPaddedRecordBufferLenght/2; i ++){
		...
		for(i = 1; i< zeroPaddedRecordBufferLenght/2; i ++){ // @TODO index BUG?
			
			
			if(FFTData[i] > FFTData[i-1] && FFTData[i] > FFTData[i+1]){

				if( ((i * freqResolution) > 0) && (FFTData[i] / peakIntensity) * 10000 >2000) {
		...
		fprintf(stdout, "Peak Finding Done\n");
		globalPeakCount = internalPeakCount;
		
		//// Data logging	(char* reading
		//char *filePath = "/home/pi/Documents/FFT_PLOT.dat";
		...
		//{
			//for(i = 0; i < zeroPaddedRecordBufferLenght/4; i++){
				...
				//fputs(frequencyReading,file);	
				//fputs("\t",file);
				//fputs(intensityReading,file);	
				//fputs("\n",file);
			//}
...
void *playTone(void *f)
{
		...
		for (i = 0; i < globalPeakCount; i++){
			...
			volume = (foundPeakFrequency[1][i] * 0.2) / 10000;
		//	printf("Volume: %f\n", volume);
